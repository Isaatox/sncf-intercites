...
import IndicateursProgression from './components/IndicateursProgression';
import './App.css';
...
function App() {
  ...
  const [villesRestantes, setVillesRestantes] = useState([...villesInitiales]);
  const [villeActuelle, setVilleActuelle] = useState(null);
  const [score, setScore] = useState(0);
  const [villesRepondues, setVillesRepondues] = useState([]);

  useEffect(() => {
    if (jeuCommence) {
      choisirVilleAleatoire(villesInitiales);
    }
  }, [jeuCommence]);

  const handleStart = (pseudoJoueur) => {
    setPseudo(pseudoJoueur);
    setScore(0);
  ...
  const handleWrongAnswer = () => {
    setVillesRepondues(prev => [...prev, "incorrect"]);
    choisirVilleSuivante();
  };

  const choisirVilleSuivante = () => {
